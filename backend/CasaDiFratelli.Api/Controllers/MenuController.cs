...
public class MenuController : ControllerBase
{
    ...
    private async Task<List<object>> ReadMenuItemsAsync()
    {
        ...
        while (await reader.ReadAsync())
        {
            items.Add(new
            {
                ...
                NameRu = reader.IsDBNull(3) ? string.Empty : reader.GetString(3),
                DescriptionBg = reader.IsDBNull(4) ? string.Empty : reader.GetString(4),
                DescriptionEn = reader.IsDBNull(5) ? string.Empty : reader.GetString(5),
                DescriptionRu = reader.IsDBNull(6) ? string.Empty : reader.GetString(6),
                ImageUrl = reader.IsDBNull(7) ? string.Empty : reader.GetString(7),
                Weight = reader.IsDBNull(8) ? string.Empty : reader.GetString(8),
                Price = reader.IsDBNull(9) ? 0m : Convert.ToDecimal(reader.GetValue(9)),
                Department = reader.IsDBNull(10) ? "Kitchen" : reader.GetString(10),
                Category = reader.IsDBNull(11) ? "main" : reader.GetString(11),
                IsActive = ReadBoolean(reader, 12, true),
                NotifySubscribers = ReadBoolean(reader, 13)
            });
        }
    ...
    public async Task<IActionResult> Create([FromBody] MenuItem item)
    {
        try
        {
            ...
            item.NameRu = string.IsNullOrWhiteSpace(item.NameRu) ? item.NameBg : item.NameRu.Trim();
            item.DescriptionBg = item.DescriptionBg?.Trim() ?? string.Empty;
            item.DescriptionEn = item.DescriptionEn?.Trim() ?? string.Empty;
            item.DescriptionRu = item.DescriptionRu?.Trim() ?? string.Empty;
            ...
            command.CommandText = """
                ...
                ("NameBg", "NameEn", "NameRu", "DescriptionBg", "DescriptionEn", "DescriptionRu", "ImageUrl", "Weight", "Price", "Department", "Category", "IsActive", "NotifySubscribers", "CreatedAtUtc")
                VALUES
                (@nameBg, @nameEn, @nameRu, @descriptionBg, @descriptionEn, @descriptionRu, @imageUrl, @weight, @price, @department, @category, @isActive, @notifySubscribers, now())
                RETURNING "Id";
                """;
            AddParameter(command, "@nameBg", item.NameBg);
            AddParameter(command, "@nameEn", item.NameEn);
            AddParameter(command, "@nameRu", item.NameRu);
            AddParameter(command, "@descriptionBg", item.DescriptionBg);
            AddParameter(command, "@descriptionEn", item.DescriptionEn);
            AddParameter(command, "@descriptionRu", item.DescriptionRu);
    ...
    public async Task<IActionResult> Update(int id, [FromBody] MenuItem updated)
    {
        try
        {
            ...
            updated.NameRu = string.IsNullOrWhiteSpace(updated.NameRu) ? updated.NameBg : updated.NameRu.Trim();
            ...
            updated.DescriptionRu = updated.DescriptionRu?.Trim() ?? string.Empty;
